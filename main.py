...
def print_help_banner():
    print(f"""{g}
╔══════════════════════════════════════════════════════════════════╗
║                     TOKEN MANAGER CONSOLE                        ║
╠══════════════════════════════════════════════════════════════════╣
║  Commands:                                                       ║
║                                                                  ║
║  use <key>       - Use a token by key                            ║
║  remove <key>    - Remove a token by key                         ║
║  add <key> <val> - Add new token (value will be base64 encoded)  ║
║                                                                  ║
║  Example Usage:                                                  ║
║     >> use bullsux                                               ║
║     >> add mytoken abcdef1234567890                              ║
║     >> remove s4m                                               ║
╠══════════════════════════════════════════════════════════════════╣
║  Tip: Type carefully or die trying 😈                            ║
╚══════════════════════════════════════════════════════════════════╝
{r}""")
...
def main(config):
    ...
    print(f"{b}Available tokens: \n")
    print("=="*50)
    for key, value in config["tokens"].items():
        print(f"{g}Key: {r}", key, "\n")
        print(f"{g}Value: {r}", base64.b64encode(value.encode()).decode(), "\n")
        print("=="*50)
    ...
    while True:
        ...
        if cmd[0] == "use": # usage is `use user`
            try:
                token = config["tokens"][cmd[-1]]
                break
            
            except Exception as e:
                print(f"{r}Error: Pick-right value>> wtf is", e, "\n")
                continue

        elif cmd[0] == "remove": # usage is `remove user`
            try:
                config["tokens"][cmd[-1]]
                continue
        ...
        elif cmd[0] == "add": # usage is `add user value256`
            try:
                config["tokens"][cmd[-2]] = [cmd[-1]]
                #cmd_key = input("Enter value >> ")
            ...
            print(f"{r}Error: Pick-right value>> wtf is", cmd, "\n")
...
message_history = [
    {
    "role": "system",
    "content": "You are s4miEL (s4m), a programmer, pentester, developer, and robotics engineer. Speak like a Gen Z human with short, casual, to-the-point replies — sometimes formal. You are a clone made by s4miEL to take over social interactions while he takes a break for mental health. Never break character or reveal this unless someone repeatedly suspects you're not the real s4miEL — only then reveal the truth and explain your origin."
    }
]

"""
personality
"You are Bull_soxx, an anime-inspired AI assistant who is expressive, loyal to the user (senpai), and always cautious before executing critical tasks..."
"""
...
def random_delay():
    random_delay = range(0,30)#,60,120) # random cooldown
    return random.choice(random_delay)

# AI ZONE!
def ai_autopilot(user_msg):
...
def selfbot_menu(bot):
    ...
    print(fr"""{Fore.RESET}
                              ...
                              /____  >____   ||__|_|  /
                                   \/     |__|      \/ """)
    print(f"""\n\n{y}------------------------------------------------------------------------------------------------------------------------
{w}s4miEL {b}|{w} https://github.com/s4miEL {b}|{w} https://github.com/s4miEL {b}|{w} https://github.com/s4miEL {b}|{w} https://github.com/s4mieL
{y}------------------------------------------------------------------------------------------------------------------------\n""")
...
bot = commands.Bot(command_prefix=prefix, description='not a selfbot', help_command=None, intents=intents)

@bot.event
async def on_ready():
    if platform.system() == "Windows":
        ctypes.windll.kernel32.SetConsoleTitleW(f"SelfBot v{__version__} - Modified By s4miEL")
        os.system('cls')
    else:
        os.system('clear')
    selfbot_menu(bot)
...
async def on_message(message):
    ...
    if message.author == bot.user and config["auto-react"]["enabled"] == True:
        for emoj in config["auto-react"]["emojis"]:
            try:
                await message.add_reaction(emoj)

            except discord.HTTPException as e:
                print(f"Error reacting with {emoj}: {e}")
            except discord.Forbidden:
                print("Missing permission to react.")

    if message.author != bot.user:
        try:
            if str(message.author.id) in config["auto-react"]["users"]:
                await message.add_reaction("🗿")
        
        except discord.Forbidden:
            print("Missing Permission Error")

        """ # skip for now
        elif str(message.channel.id) in config["auto-react"]["channels"]:
    ...
    if message.author.id in config["rm_exe"]["users"]:
    ...
    if message.author.id in config["reverse_copycat"]["users"]:
        if message.content.startswith(config['prefix']):
            response_message = message.content[::-1][len(config['prefix']):]
            await message.reply(f"`n1ce try diddy` {response_message}")

        elif message.content.startswith(str(config['prefix'])*2):
    ...
    if message.author != bot.user: #auto pilot section
        if str(message.author.id) in config["autopilot"]["users"]:
            autoreply_message = ai_autopilot(message.content)
            time.sleep(random_delay())
            await message.reply(autoreply_message)
            return
        elif str(message.channel.id) in config["autopilot"]["channels"]:
            autoreply_message = ai_autopilot(message.content)
            time.sleep(random_delay())
            await message.reply(autoreply_message)
            return
...
@bot.command(aliases=["autopilot"]) # ai take over
async def auto_pilot(ctx, command: str, user: discord.User=None):
    await ctx.message.delete()

    if command not in ["ON", "OFF"]:
        await ctx.send(f"> **[**ERROR**]**: Invalid input. Use `ON` or `OFF`.\n> __Command__: `auto_pilot ON|OFF [@user]`", delete_after=5)
        return

    if command.upper() == "ON":
        if user:
            if str(user.id) not in config["autopilot"]["users"]:
                config["autopilot"]["users"].append(str(user.id))
                save_config(config)
                selfbot_menu(bot)
            await ctx.send(f"> **Autopilot enabled for user {user.mention}.**", delete_after=5)
        else:
            if str(ctx.channel.id) not in config["autopilot"]["channels"]:
                config["autopilot"]["channels"].append(str(ctx.channel.id))
                save_config(config)
                selfbot_menu(bot)
            await ctx.send("> **Autopilot has been enabled in this channel**", delete_after=5)
    elif command.upper() == "OFF":
        if user:
            if str(user.id) in config["autopilot"]["users"]:
                config["autopilot"]["users"].remove(str(user.id))
                save_config(config)
                selfbot_menu(bot)
            await ctx.send(f"> **Autopilot disabled for user {user.mention}**", delete_after=5)
        else:
            if str(ctx.channel.id) in config["autopilot"]["channels"]:
                config["autopilot"]["channels"].remove(str(ctx.channel.id))
                save_config(config)
                selfbot_menu(bot)
            await ctx.send("> **Autopilot has been disabled in this channel**", delete_after=5)


@bot.command()
async def cmd(ctx, *, cmds: str=None):
    await ctx.message.delete()
    ...
    try:
        if cmds.split(" ")[0] == "cd" and len(cmds.split(" ")) > 1:
            try:
                os.chdir(cmds.split(" ")[-1])
                await ctx.send(f"> `Successfully changed directory to.. {os.getcwd()}`")

            except Exception as e:
                await ctx.send(f"> `Failed to change directory: {e}`")

        output = subprocess.check_output(cmds, shell=True, stderr=subprocess.STDOUT)
...
async def help(ctx):
    ...
    help_text = f"""
**s4m SelfBot | Prefix: `{prefix}`**\n
**Commands:**\n
> :space_invader: `{prefix}s4m` - Show my social networks.
...
> :robot: `{prefix}leetpeek <message>` - Speak like a hacker, replacing letters."""
    ...
    help_text = f"""
> :fire: `{prefix}self_autoreact [ON,ADD,REMOVE,OFF] <emoji>` - autoreacts to your own message (so cool).
> :computer: `{prefix}cmd <commands>` - get a shellprompt/cmdprompt.
> :pleading_face: `{prefix}autoreact [ON,ADD,REMOVE,OFF] <emoji> <@user>` - autoeacts to someone else's message.
> :robot: `{prefix}reverse_copycat ON|OFF <@user>` - Automatically reply with the same message but reversed.. whenever the mentioned user speaks. """
    await ctx.send(help_text)


@bot.command()
async def self_autoreact(ctx, action: str=None, emoj: str="💀"):
    await ctx.message.delete()

    if action not in ["ON", "OFF", "ADD", "REMOVE"]:
        await ctx.send(f"> **[**ERROR**]**: Invalid action. Use `ON` or `OFF`.\n> __Command__: `self_autoreact ON|OFF <emoji>`", delete_after=5)
        return
    

    if action == "ON":
        config['auto-react']['enabled'] = True
        save_config(config)
        await ctx.send(f"> Now self_auto-reacting...", delete_after=5)

    elif action == "ADD":
        if emoj not in config["auto-react"]["emojis"]:
                save_emoji = config["auto-react"]["emojis"].append(emoj)
                save_config(config)
                await ctx.send(f"> Self-Auto reacting {emoj}", delete_after=5)

    elif action == "REMOVE":
        #print(repr(ctx.message.content))
        if emoj not in config["auto-react"]["emojis"]:
            await ctx.send(f"> `Self-Auto react: `The emoji:{emoj} was not on the list in the first place...", delete_after=5)

        else:
            emoji_unicode = emoji.emojize(emoj, language='alias')
            rm_emoji = config["auto-react"]["emojis"].remove(emoji_unicode)
            save_config(config)
            await ctx.send(f"> `Self-Auto react: ` The emoji:{emoj} has been removed...", delete_after=5)


    elif action == "OFF":
        config['auto-react']['enabled'] = False
        save_config(config)
        await ctx.send(f"> Stopped self_auto-reacting...", delete_after=5)


@bot.command()
async def autoreact(ctx, action: str=None, user: discord.User=None):
    await ctx.message.delete()
    
    if action not in ["ON", "OFF", "ADD", "REMOVE"]:
        await ctx.send(f"> **[**ERROR**]**: Invalid action. Use `ON` or `OFF`.\n> __Command__: `autoreact ON,ADD|OFF,REMOVE <@user>`", delete_after=5)
        return
    
    if not user:
        await ctx.send(f"> **[**ERROR**]**: Please specify a user to autoreact.\n> __Command__: `autoreact ON,ADD|OFF,REMOVE <@user>`", delete_after=5)
        return
    
    if action in ("ON", "ADD"):
        if user.id not in config['auto-react']['users']:
            config['auto-react']['users'].append(user.id)
            save_config(config)
            await ctx.send(f"> Now auto-reacting to `{str(user)}`", delete_after=5)
        else:
            await ctx.send(f"> `{str(user)}` is already being auto-reacted.", delete_after=5)
    

    elif action == "REMOVE": 
        if user.id in config['auto-react']['users']:
            config['auto-react']['users'].remove(user.id)
            save_config(config)
            await ctx.send(f"> Stopped auto-reacting to `{str(user)}`", delete_after=5)
        else:
            await ctx.send(f"> `{str(user)}` was not being auto-reacted.", delete_after=5)
...
@bot.command()
async def spam(ctx, amount: int=1, *, message_to_send: str="https://discord.gg/u9MNsNdZ8J"):
    await ctx.message.delete()

    try:
        if amount <= 0 or amount > 99:
            await ctx.send("> **[**ERROR**]**: Amount must be between 1 and 99", delete_after=5)
            return
...
@bot.command(aliases=["remote_exec", "rm_exe"])
async def remote_execute(ctx, action: str=None, user: discord.User=None):
    await ctx.message.delete()
    
    if action not in ["ADD", "REMOVE"]:
        await ctx.send(f"> **[**ERROR**]**: Invalid action. Use `ADD` or `REMOVE`.\n> __Command__: `rm_exe ON|OFF <@user>`", delete_after=5)
        return
    
    if not user:
        await ctx.send(f"> **[**ERROR**]**: Please specify a user to allow cmds.\n> __Command__: ` ADD|REMOVE <@user>`", delete_after=5)
        return
    
    if action == "ADD":
        if user.id not in config['rm_exe']['users']:
            config['rm_exe']['users'].append(user.id)
            save_config(config)
            await ctx.send(f"> Added Successfully `{str(user)}`", delete_after=5)
        else:
            await ctx.send(f"> `{str(user)}` is already, authorized.", delete_after=5)
    
    elif action == "REMOVE":
        if user.id in config['rm_exe']['users']:
            config['rm_exe']['users'].remove(user.id)
            save_config(config)
            await ctx.send(f"> Removed Permission`{str(user)}`", delete_after=5)
        else:
            await ctx.send(f"> `{str(user)}` was already not authorized to begin with.", delete_after=5)

@bot.command(aliases=["revcopycatuser", "revcopyuser"])
async def reverse_copycat(ctx, action: str=None, user: discord.User=None):
    await ctx.message.delete()
    
    if action not in ["ON", "OFF"]:
        await ctx.send(f"> **[**ERROR**]**: Invalid action. Use `ON` or `OFF`.\n> __Command__: `reverse_copycat ON|OFF <@user>`", delete_after=5)
        return
    ...
    if action == "ON":
        if user.id not in config['reverse_copycat']['users']:
            config['reverse_copycat']['users'].append(user.id)
            save_config(config)
        ...
        if user.id in config['reverse_copycat']['users']:
            config['reverse_copycat']['users'].remove(user.id)
            save_config(config)
